# -*- coding: utf-8 -*-
import Lib.tools.Ctu_InstanceSegmentation import Ctu_InstanceSegmentation,Ctu_InstanceSegmentation_Predictor,Ctu_InstanceSegmentation_Predictor_Work
...
if __name__ == '__main__':
    ...
    if func_type=='train':
        work_thread='1'
        # 训练
        ctu_train = Ctu_InstanceSegmentation(
            use_gpu='0',min_size=600,max_size=1000,mean = [123.15163084, 115.90288257, 103.0626238],
                 ...
                 project_name='ctu_ins',result_model='ctu_result_modeltest'
        )
        for each_key in ctu_train.func_signal.keys():
            ctu_train.connect_signal(each_key,get_recv_data_ins_tools)
        init_res = ctu_train.init_model(data_dir=r'dataset/DataSet_Tablet/DataJson',train_split=0.9,batch_size=1,model_name='fasterrcnn_resnet18',alpha=0.25,pre_model=None,aux=None,encoding='gbk',n_processes=None)
        ...
        simple = False
        ctu_predict = Ctu_InstanceSegmentation(use_gpu='0',lic_file="license.lic", time_sleep=5, log_flag='1', thread_check='1',project_name='ctu_ins')
        for each_key in ctu_predict.func_signal.keys():
            ctu_predict.connect_signal(each_key,get_recv_data_ins_tools)
        load_res = ctu_predict.load_model(params_file='ctu_result_model/ctu_ins/ctu_params.json', use_best=True, snid='')
        if load_res['return_value']=='1':
            ...
            for each_data in data_list_split:
                ...
                for each_id in range(0,len(res_list['predict_output'])):
                    for each_bbox in res_list['predict_output'][each_id]['target_list']:
                        print(each_bbox)
        ...
        ctu_fps = Ctu_InstanceSegmentation(use_gpu='0',lic_file="license.lic", time_sleep=5, log_flag='1', thread_check='1',project_name='ctu_ins')
        for each_key in ctu_fps.func_signal.keys():
            ctu_fps.connect_signal(each_key,get_recv_data_ins_tools)
        load_res = ctu_fps.load_model(params_file='ctu_result_model/ctu_ins/ctu_params.json', use_best=True, snid='')
        if load_res['return_value']=='1':
        ...
        ctu_onnx = Ctu_InstanceSegmentation(use_gpu='0',lic_file="license.lic", time_sleep=5, log_flag='1', thread_check='1',project_name='ctu_ins')
        for each_key in ctu_onnx.func_signal.keys():
            ctu_onnx.connect_signal(each_key,get_recv_data_ins_tools)
        load_res = ctu_onnx.load_model(params_file='ctu_model/ins/ctu_ins_maskrcnn_resnet18/ctu_params.json', use_best=True, snid='')
        if load_res['return_value']=='1':
        ...
        simple = False
        predict_score = 0.5
        ctu_predictor=Ctu_InstanceSegmentation_Predictor(use_gpu='0', lic_file='license.lic', time_sleep=5, log_flag='1', thread_check='1', project_name='ctu_ins',result_model='ctu_logs',func=get_recv_data_ins_tools)
        load_res=ctu_predictor.load_model(params_file='ctu_result_model/ctu_ins/ctu_params.json', use_best=True)
        if load_res['return_value']=='1':
            data_list = []
            predict_cvs=[]
            predictNum=1
        ...
        ctu_model = Ctu_InstanceSegmentation_Predictor_Work(
            use_gpu = '0',
            project_name = 'ctu_ins',
            pool_dict = pool_dict[run_model],
            lic_file = 'license.lic',
            func_pool = get_recv_data_ins_tools
        )
        # 加载模型
        res_load = ctu_model.load_model('ctu_result_model/ctu_ins/ctu_params.json')
