...
| `Ctu_Classification.py` | 图像分类任务实现（含数据加载、模型定义、训练验证） |
| `Ctu_Detection.py` | 目标检测任务实现（支持主流检测框架适配） |
| `Ctu_Segmentation.py` | 语义分割任务实现（像素级分类逻辑） |
| `Ctu_InstanceSegmentation.py` | 实例分割任务实现（区分同一类别不同实例） |
