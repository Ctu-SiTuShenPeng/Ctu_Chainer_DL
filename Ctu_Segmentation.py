# -*- coding: utf-8 -*-
import Lib.tools.Ctu_Segmentation import Ctu_Segmentation,Ctu_Segmentation_Predictor,Ctu_Segmentation_Predictor_Work
...
if __name__ == '__main__':
    ...
    if func_type=='train':
        work_thread='1'
        # 训练
        ctu_train = Ctu_Segmentation(
            use_gpu='0',image_size=512,mean = [123.15163084, 115.90288257, 103.0626238],
            lic_file='license.lic', time_sleep=5, log_flag='1', thread_check='1', 
            project_name='ctu_seg',result_model='ctu_result_model_11111'
        )
        for each_key in ctu_train.func_signal.keys():
            ctu_train.connect_signal(each_key,get_recv_data_seg_tools)
        init_res = ctu_train.init_model(data_dir=r'dataset/DataSet_Tablet/DataJson',train_split=0.9,batch_size = 1, model_name="deeplab_resnet18", alpha=0.25, pre_model=None,encoding='gbk',aux=None,n_processes=None)
        if init_res['return_value']=='1':
            ctu_train.train(train_num=100, learning_rate=None, optimizer_type="adam", result_model='temp_result',work_thread=work_thread)
            if work_thread=='1':
                print('等待训练线程启动')
                time.sleep(5)
        ...
        simple = False
        ctu_predict = Ctu_Segmentation(use_gpu='0',lic_file='license.lic', time_sleep=5, log_flag='1', thread_check='1', project_name='ctu_seg',result_model='ctu_result_model')
        for each_key in ctu_predict.func_signal.keys():
            ctu_predict.connect_signal(each_key,get_recv_data_seg_tools)
        load_res = ctu_predict.load_model(params_file='ctu_result_model/ctu_seg/ctu_params.json', use_best=True, snid='')
        if load_res['return_value']=='1':
            ...
            for root, dirs, files in os.walk(r'dataset/DataSet_Tablet/DataImage'):
                for f in files:
        ...
        ctu_fps =Ctu_Segmentation(use_gpu='0',lic_file='license.lic', time_sleep=5, log_flag='1', thread_check='1', project_name='ctu_seg',result_model='ctu_result_model')
        for each_key in ctu_fps.func_signal.keys():
            ctu_fps.connect_signal(each_key,get_recv_data_seg_tools)
        load_res = ctu_fps.load_model(params_file='ctu_model/seg/ctu_seg_fastscnn/ctu_params.json', use_best=True, snid='')
        if load_res['return_value']=='1':
        ...
        ctu_onnx = Ctu_Segmentation(use_gpu='0',lic_file='license.lic', time_sleep=5, log_flag='1', thread_check='1', project_name='ctu_seg',result_model='ctu_result_model')
        for each_key in ctu_onnx.func_signal.keys():
            ctu_onnx.connect_signal(each_key,get_recv_data_seg_tools)
        load_res = ctu_onnx.load_model(params_file='ctu_model/seg/ctu_seg_fastscnn/ctu_params.json', use_best=True, snid='')
        if load_res['return_value']=='1':
        ...
        simple = False
        ctu_predictor=Ctu_Segmentation_Predictor(use_gpu='0', lic_file='license.lic', time_sleep=5, log_flag='1', thread_check='1', project_name='ctu_seg',result_model='ctu_logs',func=get_recv_data_seg_tools)
        load_res=ctu_predictor.load_model(params_file='ctu_result_model/ctu_seg/ctu_params.json', use_best=True)
        if load_res['return_value']=='1':
            data_list = []
            predict_cvs=[]
            predictNum=1
        ...
        ctu_model = Ctu_Segmentation_Predictor_Work(
            use_gpu = '0',
            project_name = 'ctu_seg',
            pool_dict = pool_dict[run_model],
            lic_file = 'license.lic',
            func_pool = get_recv_data_seg_tools
        )
        # 加载模型
        res_load = ctu_model.load_model('ctu_result_model/ctu_seg/ctu_params.json')
